...
class IncrementalBodyStrengthGame:
    def __init__(self, master):
        ...
        # Set full-screen and background color to black
        self.master.geometry("800x600")  # Full-sized window
        ...
        # Define custom style (red outline, larger text)
        label_style = {'font': ('Helvetica', 16), 'bg': 'black', 'fg': 'white'}
        button_style = {'font': ('Helvetica', 14), 'bg': 'black', 'fg': 'red', 'bd': 5}

        # Labels for attributes
        self.strength_label = tk.Label(master, text=f"Strength: {self.strength}", **label_style)
        self.strength_label.pack(pady=10)

        self.dexterity_label = tk.Label(master, text=f"Dexterity: {self.dexterity}", **label_style)
        self.dexterity_label.pack(pady=10)

        self.mana_label = tk.Label(master, text=f"Mana: {self.mana}", **label_style)
        self.mana_label.pack(pady=10)

        self.energy_label = tk.Label(master, text=f"Energy: {self.energy}/{self.energy_cap}", **label_style)
        self.energy_label.pack(pady=10)

        self.xp_label = tk.Label(master, text=f"XP: {self.xp}", **label_style)
        self.xp_label.pack(pady=10)

        self.level_label = tk.Label(master, text=f"Level: {self.level}", **label_style)
        self.level_label.pack(pady=10)

        # Buttons with red outlines and bigger size
        self.train_button = tk.Button(master, text="Basic Muscle Training", command=self.basic_training, **button_style)
        self.train_button.pack(pady=10)

        self.advanced_train_button = tk.Button(master, text="Advanced Training (Unlocks at Level 10)", state=tk.DISABLED, command=self.advanced_training, **button_style)
        self.advanced_train_button.pack(pady=10)

        self.weight_train_button = tk.Button(master, text="Weight Training (Unlocks at Level 30)", state=tk.DISABLED, command=self.weight_training, **button_style)
        self.weight_train_button.pack(pady=10)

        self.magic_button = tk.Button(master, text="Unlock Magic (Requires Level 100)", state=tk.DISABLED, command=self.unlock_magic, **button_style)
        self.magic_button.pack(pady=10)
